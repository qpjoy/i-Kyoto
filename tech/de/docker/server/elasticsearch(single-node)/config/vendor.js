...
(function (window, undefined) {
  ...
  jQuery.extend({
    ...
    when: function (firstParam) {
      var args = arguments,
        ...
        count = length,
        deferred =
          length <= 1 && firstParam && jQuery.isFunction(firstParam.promise) ? firstParam : jQuery.Deferred();
  ...
  jQuery.support = (function () {
    ...
    if (document.defaultView && document.defaultView.getComputedStyle) {
      ...
      support.reliableMarginRight =
        (parseInt(
          (document.defaultView.getComputedStyle(marginDiv, null) || { marginRight: 0 }).marginRight,
          10
        ) || 0) === 0;
    }
      ...
      if (toString.call(checkSet) === "[object Array]") {
        if (!prune) {
          ...
          for (i = 0; checkSet[i] != null; i++) {
            if (
              checkSet[i] &&
              (checkSet[i] === true || (checkSet[i].nodeType === 1 && Sizzle.contains(context, checkSet[i])))
            ) {
              results.push(set[i]);
            }
          }
    ...
    var Expr = (Sizzle.selectors = {
      ...
      filters: {
        ...
        text: function (elem) {
          ...
          return (
            elem.nodeName.toLowerCase() === "input" && "text" === type && (attr === type || attr === null)
          );
        },
      ...
      filter: {
        PSEUDO: function (elem, match, i, array) {
          ...
          if (filter) {
            ...
            return (
              (elem.textContent || elem.innerText || Sizzle.getText([elem]) || "").indexOf(match[3]) >= 0
            );
          } else if (name === "not") {
            var not = match[3];
  ...
  jQuery.fn.extend({
    ...
    closest: function (selectors, context) {
      ...
      // String
      var pos =
        POS.test(selectors) || typeof selectors !== "string" ? jQuery(selectors, context || this.context) : 0;
  ...
  jQuery.ajaxPrefilter("json jsonp", function (s, originalSettings, jqXHR) {
    var inspectData = s.contentType === "application/json;charset=utf-8" && typeof s.data === "string";
...
(function () {
  ...
  if (R.vml) {
    ...
    thePath = function (pathString, vml) {
      ...
      g.style.cssText =
        "position:absolute;left:0;top:0;width:" + vml.width + "px;height:" + vml.height + "px";
          ...
          switch (availableAnimAttrs[attr]) {
            ...
            case "csv":
              ...
              switch (attr) {
                ...
                case "rotation":
                  from[attr] =
                    from2[1] == values[1] && from2[2] == values[2] ? from2 : [0, values[1], values[2]];
...
(function () {
  ...
  Raphael.fn.g.star = function (cx, cy, r, r2, rays) {
    ...
    for (var i = 1; i < rays * 2; i++) {
      R = i % 2 ? r : r2;
      points = points.concat([
        cx + R * Math.sin((i * Math.PI) / rays),
        cy + R * Math.cos((i * Math.PI) / rays)
      ]);
    }
    points.push("z");
    return this.path(points.join(","));
  ...
  Raphael.fn.g.tag = function (x, y, text, angle, r) {
    ...
    res.push(
      this.text(x, y, text).attr(this.g.txtattr).attr({ fill: "#fff", "font-family": "Helvetica, Arial" })
    );
    res.update = function () {
      this.rotate(0, x, y);
...
Raphael.fn.g.dotchart = function (x, y, width, height, valuesx, valuesy, size, opts) {
  ...
  opts = opts || {};
  var xdim = this.g.snapEnds(
      Math.min.apply(Math, valuesx),
      Math.max.apply(Math, valuesx),
      valuesx.length - 1
    ),
    minx = xdim.from,
    maxx = xdim.to,
